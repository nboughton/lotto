<template>
  <div id="app" class="section">
    ...
    <div class="box">
      ...
      <b-table :data="tables.main.data" :default-sort="[ 'num', 'desc' ]" :bordered="tables.main.cfg.isBordered" :striped="tables.main.cfg.isStriped"
        :narrowed="tables.main.cfg.isNarrowed" :checkable="tables.main.cfg.isCheckable" :paginated="tables.main.cfg.isPaginated">
      ...
      runQuery: function () {
        this.$http.get("/api/query", { params: this.qParams }).then(resp => { return resp.json() }).then(d => {
          for (var ball = 0; ball < d.data.timeSeries.datasets.length; ball++) {
            ...
            d.data["freqDist"].datasets[ball].borderWidth = 10
          }
